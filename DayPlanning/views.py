        ...
        try:
            ...
            for idx, row_data in enumerate(rows, start=1):
                try:
                    ...
                    # Create ModelMasterCreation record
        ...
        try:
            ...
            with transaction.atomic():
                ...
                total_stock_obj = TotalStockModel.objects.create(
                    ...
                    balance_quantity=total_batch_quantity,
                    assigned_quantity=0,
...
class TrayIdListAPIView(APIView):
    def get(self, request):
        ...
        tray_ids = TrayId.objects.filter(batch_id__batch_id=batch_id)
        data = [
            {
                's_no': idx + 1,
        ...
        try:
            ...
            for tray in trays:
                ...
                if draft_id:
                    ...
                    DraftTrayId.objects.filter(id=draft_id).update(
                        tray_id=tray_id,
                        tray_quantity=int(tray_quantity),
                        lot_id=lot_id,
                        batch_id=batch_instance,
                    ...
                    DraftTrayId.objects.update_or_create(
                        tray_id=tray_id,
                        defaults={
                            'lot_id': lot_id,
                            'batch_id': batch_instance,
...
class DraftTrayIdListAPIView(APIView):
    def get(self, request):
        ...
        if not batch_id:
            return JsonResponse({'success': False, 'error': 'Missing batch_id'}, status=400)
        draft_trays = DraftTrayId.objects.filter(batch_id__batch_id=batch_id)
        if ids:
...
@method_decorator(csrf_exempt, name='dispatch')
class UpdateBatchQuantityAPIView(APIView):
    def post(self, request):
        try:
